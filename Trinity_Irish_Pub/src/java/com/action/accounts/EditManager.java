...
public class EditManager extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String msg="Account Not Updated";
        try {
            ...
            String as="";
            out.println(id+" "+uname+"\t"+pw+" "+name+" "+email+" "+des+" "+ph+" ");
            if(roles!=null)
            {
                for(int i=0;i<roles.length;i++)
                    as+=roles[i]+",";
            }  
            PreparedStatement ps=DBConnection.getPrepare("update manager set uname=?,password=?,name=?,email=?,desg=?,phone=?,roles=? where id="+id);
